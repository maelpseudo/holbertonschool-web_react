...
log(
  ex("src/assets/holberton-logo.jpg"),
...
// 2) Titre & icône
const titleOK = /<title>\s*Holberton - School dashboard\s*<\/title>/i.test(
  html
);
log(titleOK, "index.html: titre exact");

const iconTags = [...html.matchAll(/<link[^>]+rel=["']icon["'][^>]*>/gi)].map(
  (m) => m[0]
);
log(
  iconTags.length === 1,
  `index.html: une seule balise icon (trouvé=${iconTags.length})`
);
const iconIsLogo = /href=["']\/holberton-logo\.jpg["']/i.test(
  iconTags[0] || ""
);
...
log(
  /alt\s*=\s*["']holberton logo["']/i.test(app),
...
const footerOK =
  ...
  /Copyright\s*\{[^}]*getFullYear\(\)[^}]*\}\s*-\s*holberton\s+School/i.test(
    app
  ) ||
...
try {
  ...
  const hasJestDep =
    (pkg.devDependencies &&
      Object.prototype.hasOwnProperty.call(pkg.devDependencies, "jest")) ||
    (pkg.dependencies &&
      Object.prototype.hasOwnProperty.call(pkg.dependencies, "jest"));
