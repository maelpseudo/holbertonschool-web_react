...
must(
  "favicon link",
  /<link[^>]+rel=["']icon["'][^>]+href=["']favicon\.ico["']/.test(indexHtml)
);
...
must(
  "App import logo",
  /import\s+logo\s+from\s+["']\.\/assets\/holberton-logo\.jpg["']/.test(app)
);
...
must(
  "Notif import close-icon",
  /import\s+\w+\s+from\s+["']\.\/assets\/close-icon\.png["']/.test(notif)
);
